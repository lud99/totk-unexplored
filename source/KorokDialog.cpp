...
void KorokDialog::SetSeed(int seed, int korokIndex)
{
    ...
    std::string path = "";
    if (SavefileIO::Get().DirectoryExists("romfs:/guide") && SavefileIO::Get().FileExists("romfs:/guide/Korok" + seedStr + ".jpg"))
        path = "romfs:/guide/Korok" + seedStr + ".jpg";
    else if (SavefileIO::Get().DirectoryExists("sdmc:/switch/totk-unexplored/guide") && SavefileIO::Get().FileExists("sdmc:/switch/totk-unexplored/guide/Korok" + seedStr + ".jpg"))
        path = "sdmc:/switch/totk-unexplored/guide/Korok" + seedStr + ".jpg";

    // Neither exists
    if (path == "")
        return;

    //Log::() "create korok dialog\n";
